...
class burgers(gym.Env):
    ...
    def __init__(self, cpu=0,
        ...
        self.n_obs_pts  = 5        # nb of observation pts
    ...
    def solve(self, a=None):
        ...
        for i in range(self.ndt_act):
            ...
            # Add control
            self.rhs[self.ctrl_pos] += self.a[0]*self.amp
    ...
    def render(self, mode="human", show=False, dump=True):
        ...
        ax.set_ylim([self.u_target-2*self.sigma,self.u_target+2*self.sigma])
        ax.set_xticks([])
        ax.set_yticks([])
        ...
        ax.add_patch(Rectangle((x,y), 0.98*self.a[0]*self.amp, 0.1,
        ...
        # Dump
        if dump: self.dump(self.path+"/fields/"+str(self.stp_plot)+".dat")
